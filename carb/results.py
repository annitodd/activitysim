...
def get_scenario_resutls(policy_name, scenario, settings):
...
#     skims = omx.open_file(os.path.join(path, 'data', 'skims.omx'),'r')
    ...
    households = pd.read_csv(os.path.join('output','final_households.csv'), 
                         index_col = 'household_id')
    persons = pd.read_csv(os.path.join('output','final_persons.csv'), 
                     index_col = 'person_id')
    trips = pd.read_csv(os.path.join('output','final_trips.csv'), 
                   index_col = 'trip_id')

    skims = omx.open_file(os.path.join('data', 'skims.omx'),'r')
    
    df = add_results_variables(settings, trips, households, skims)
